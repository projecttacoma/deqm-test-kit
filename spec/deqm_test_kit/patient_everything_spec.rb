...
RSpec.describe DEQMTestKit::PatientEverything do
    ...
    describe 'Patient/<id>/$everything successful test' do
        ...
        it 'passes for valid request to Patient/<id>/$everything' do
            stub_request(:post, "#{url}/").with(body: single_patient_bundle, headers: single_headers).to_return(status: 200, body: "", headers: {})
            stub_request(:post, "#{url}/Patient/test-patient/$everything").to_return(status: 200, body: single_patient_response, headers: {})
    ...
    describe 'Patient/$everything successful test' do
        ...
        it 'passes for valid request to Patient/$everything' do
            stub_request(:post, "#{url}/").with(body:  multiple_patient_bundle, headers: mult_headers).to_return(status: 200, body: "", headers: {})
            stub_request(:post, "#{url}/Patient/$everything").to_return(status: 200, body: multiple_patient_response, headers: {})
            result = run(test, url: url)
            expect(result.result).to eq('pass')
        end
